...
void MotionControl_IrqHandler(){
    ...
    if(uncontroll_count > MC_UNCONTROLLABLE_LIMIT){
        uncontroll_count = 0;
        ApMpData[apmpBank].cmd = AP_MP_CMD_IDLE;
    }
    ...
    if(ApMpData[apmpBank].cmd == AP_MP_CMD_IDLE){
    ...
    if(ApMpData[apmpBank].cmd == AP_MP_CMD_IDLE && (MC_state.state == MC_STATE_START || MC_state.state == MC_STATE_RUN)){
        MC_state.state = MC_STATE_STOP;
    }
    // ステート制御
    switch(MC_state.state){
        case MC_STATE_INIT: // 初期化
            ...
            for (int i=0; i<3; i++) {
                ...
                //imu_data.theta[i] = 0.0f;
#if 0
                imu_data.gyro_scale_gain = 1.0f + (float)((int32_t)ApMpData[apmpBank].imu_gyro_scale_gain - 32768)*GYRO_SCALE_GAIN_INT16t_TO_FLOAT; //機体ごとのスケールゲインをAPからもらう
            }
        ...
        case MC_STATE_START://SVを駆動モードに入れてStateをrunに変更する
            ...
            MECP.wheel_radius_r = ((float)ApMpData[apmpBank].wheel_radius_right) / 1000000.0f;
            MECP.wheel_radius_l = ((float)ApMpData[apmpBank].wheel_radius_left ) / 1000000.0f;
            MECP.tread =          ((float)ApMpData[apmpBank].tread )             / 100000.0f;
            imu_data.gyro_scale_gain = 1.0f + ((float)((int32_t)(ApMpData[apmpBank].imu_gyro_scale_gain)) - 32768) * GYRO_SCALE_GAIN_INT16t_TO_FLOAT; //機体ごとのスケールゲインをAPからもらう
