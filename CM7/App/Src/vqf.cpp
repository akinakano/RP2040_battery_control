...
int VQF::getDebugCountThGyro() const
{
    return params.debugCountThGyro;
}

int VQF::getDebugCountThAcc() const
{
    return params.debugCountThAcc;
}
...
void VQF::setQuat(const vqf_real_t gyrQuat[4], const vqf_real_t accQuat[4])
{
    std::copy(gyrQuat, gyrQuat+4, state.gyrQuat);
    std::copy(accQuat, accQuat+4, state.accQuat);
}

void VQF::getQuat(vqf_real_t gyrQuat[4], vqf_real_t accQuat[4])
{
    std::copy(state.gyrQuat, state.gyrQuat+4, gyrQuat);
    std::copy(state.accQuat, state.accQuat+4, accQuat);
}
